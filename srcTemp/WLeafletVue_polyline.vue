...
import { LMap, LTileLayer, LControl, LControlAttribution, LControlZoom, LControlScale, LLayerGroup, LMarker, LCircleMarker, LPolygon, LPolyline, LImageOverlay, LTooltip } from 'vue2-leaflet'
...
export default {
    ...
    methods: {
        ...
        changeItems: function(from) {
            // console.log('methods changeItems', from)
            ...
            each(vo.polygonSets, (v) => {
                items.push({
                    ...v,
                    type: 'polygon',
                })
            })
            each(vo.polylineSets, (v) => {
                items.push({
                    ...v,
                    type: 'polyline',
                })
            })
            each(vo.geojsonSets, (v) => {
                items.push({
                    ...v,
                    type: 'geojson',
                })
            })
            each(vo.contourSets, (v) => {
                items.push({
                    ...v,
                    type: 'contour',
                })
            ...
            // //check, 外部傳入opt時, 內部items會因vo.pointSets等變更而觸發, 但此時vo.items尚未更新, 故isEqual會判斷為true而return, 導致未更新, 故不使用isEqual
            // if (isEqual(vo.items, items)) {
            //     return
            // }
            vo.items = items
            // console.log('vo.items', cloneDeep(vo.items))

        },

        updateItems: function(items) {
            // console.log('methods updateItems', cloneDeep(items))
        ...
        updateLatLng: function(ev) {
            // console.log('methods updateLatLng', ev)

            let vo = this

            //showLoc
            vo.showLoc = {
                lat: dig(ev.latlng.lat, 7),
                lng: dig(ev.latlng.lng, 7),
            }

        },

        contourRefresh: function(v, contourSet, kcontourSet) {
            // console.log('methods contourRefresh', v, contourSet, kcontourSet)

            let vo = this

            //legend
            set(vo, `contourSets[${kcontourSet}].legend`, v.legend)

            //legendMsg
            set(vo, `contourSets[${kcontourSet}].legendMsg`, v.legendMsg)
        ...
        clickImage: function(ev, imageSet, kimageSet, imageSets) {
            ...
            //fun
            let fun = get(imageSet, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, imageSet, kimageSet, imageSets)
            }
        ...
        clickPoint: function(ev, point, kpoint, pointSet, kpointSet, pointSets) {
            ...
            //fun
            let fun = get(point, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, point, kpoint, pointSet, kpointSet, pointSets)
            }
        ...
        clickPolygon: function(ev, polygonSet, kpolygonSet, polygonSets) {
            ...
            //fun
            let fun = get(polygonSet, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, polygonSet, kpolygonSet, polygonSets)
            }
        ...
        clickPolyline: function(ev, polylineSet, kpolylineSet, polylineSets) {
            ...
            //fun
            let fun = get(polylineSet, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, polylineSet, kpolylineSet, polylineSets)
            }
        ...
        clickGeojson: function(ev, geojsonSet, kgeojsonSet, geojsonSets) {
            ...
            //fun
            let fun = get(geojsonSet, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, geojsonSet, kgeojsonSet, geojsonSets)
            }

        },

        clickContour: function(ev, contourSet, kcontourSet, contourSets) {
            // console.log('methods clickContour', ev, contourSet, kcontourSet, contourSets)

            let vo = this

            //fun
            let fun = get(contourSet, 'funSetsClick')
            if (isfun(fun)) {
                fun(ev, contourSet, kcontourSet, contourSets)
            }
        ...
        panTo: function(center, zoom) {
            // console.log('methods panTo', center, zoom)
            ...
            //panTo
            mapObject.panTo(center, zoom)

        },

        centerTo: function(center, zoom) {
            // console.log('methods centerTo', center, zoom)
        ...
        popupPoint: function(opt) {
            // console.log('methods popupPoint', opt)
            ...
            //latLng
            let latLng = get(opt, 'latLng')
            if (!isarr(latLng)) {
                return
            }

            //html
            let html = get(opt, 'html')
            if (!isstr(html)) {
                return
            }
        ...
        modifyItemsVisible: function(items) {
            // console.log('methods modifyItemsVisible', items)
